...
class Bipartite_Graph(Graph):
    ...
    def __str__(self):
        return f'Bipartite graph with {len(self.vertices_A)} vertices in A, {len(self.vertices_B)} vertices in B and {self.num_edges} edges.'
    
    def add_vertices(self, vertex_names, partition):
        for vertex_name in vertex_names:
            if partition == 'A' and vertex_name not in self.vertices_A:
                self.vertices_A.append(vertex_name)
                self.vertices[vertex_name] = Vertex(vertex_name)
            elif partition == 'B' and vertex_name not in self.vertices_B:
                self.vertices_B.append(vertex_name)
                self.vertices[vertex_name] = Vertex(vertex_name)
    
    def add_edge(self, vertex_nameA, vertex_nameB):
        if vertex_nameA in self.vertices_A and vertex_nameB in self.vertices_B:
            super().add_edge(vertex_nameA, vertex_nameB)
        elif vertex_nameA in self.vertices_B and vertex_nameB in self.vertices_A:
            super().add_edge(vertex_nameB, vertex_nameA)
        else:
            # ignore if the vertices are not in the right partitions
            pass
...
G.add_edge('u','x')
print(G)
#Graph with 4 vertices and 3 edges.
G.add_edge('x','y')
print(G)
#Graph with 5 vertices and 4 edges.
G.add_edge('z','a')
print(G)
#Graph with 7 vertices and 5 edges.
